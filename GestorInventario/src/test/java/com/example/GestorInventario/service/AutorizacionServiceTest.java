...
public class AutorizacionServiceTest {
    ...
    void obtenerRolUsuarioConectado_devuelveRolCorrectamente() {
        ...
        // simular que el usuario con ID 100 tiene rol 2
        Map<String, Object> usuarioMap = new HashMap<>();
        usuarioMap.put("idRol", 2);
    ...
    void validarRol_devuelveOK() {
        // mockear obtenerRolUsuarioConectado para devolver rol esperado 2
        AutorizacionService spyService = spy(autorizacionService);
        doReturn(2).when(spyService).obtenerRolUsuarioConectado(3);

        ResponseEntity<?> response = spyService.validarRol(3, 2);
